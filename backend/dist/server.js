...
process.on('unhandledRejection', (reason) => {
    logger_config_1.logger.error('CRITICAL: Unhandled Promise Rejection detected', { reason });
    process.exit(1);
});
async function bootstrap() {
    try {
        ...
        server = app_1.default.listen(env_config_1.env.PORT, () => {
            ...
            logger_config_1.logger.info(`API Base Endpoint: http://localhost:${env_config_1.env.PORT}/api/v1`);
        });
    }
    catch (error) {
